...
wa_dat_scale <- wa_dat |> 
    dplyr::select(sample_ID, lower_depth, SOC_stock_spline, 
                  site, CHM, DTM, geomorphons, GEO, LITH,
                  WIP, EVT, EVI, NDVI, NDYI, VPD, 
                  dev_1000, grad_1000, dev_300, dev_50, grad_300, grad_50,
...
```{r}
wa_dat_scale_num <- wa_dat_scale |> 
    dplyr::select((where(is.numeric)))|> as.matrix()

corplot_scale <- ggcorrplot(cor(wa_dat_scale_num), method = "square", type = "full", lab = T, lab_size = 3)
...
modw <- dredge(gmodw, rank = "AIC", extra = "R^2", m.lim = c(3,8), fixed = c("CHM", "WIP" , "MAP", "HLI"))
...
modu <- dredge(gmodu, rank = "AIC", extra = "R^2", m.lim = c(3,8), fixed = c("CHM", "WIP" , "MAP", "HLI"))
...
qqline(resid(mod7d1))
qqnorm(resid(modw), col = as.factor(wa_dat_scale_wet$sample_ID))
qqline(resid(modw))
qqnorm(resid(modu), col = as.factor(wa_dat_scale_upl$sample_ID))
qqline(resid(modu))
...
# caret::R2(pred = fitted(mod7d1), obs = log10(wa_dat_scale$SOC_stock_spline), formula = "traditional")
# caret::postResample(pred = 10**fitted(mod7d1), obs = (wa_dat_scale$SOC_stock_spline))
...
readr::write_csv(PI_trans, file = "SOIL CARBON/All_WA/data/dataframes/PI_Full_Depths_Transformed_Dataframe.csv")
readr::write_csv(PIw_trans, file = "SOIL CARBON/All_WA/data/dataframes/PIwet_Full_Depths_Transformed_Dataframe.csv")
readr::write_csv(PIu_trans, file = "SOIL CARBON/All_WA/data/dataframes/PIupl_Full_Depths_Transformed_Dataframe.csv")


PI_rmse <- sqrt(abs(sum((PI_trans$fit_t - PI_trans$act)**2))/nrow(wa_dat_scale))
PI_rmse_upr <- sqrt(abs(sum((PI_trans$upr_t - PI_trans$act)**2))/nrow(wa_dat_scale))
PI_rmse_lwr <- sqrt(abs(sum((PI_trans$lwr_t - PI_trans$act)**2))/nrow(wa_dat_scale))
PI_rmse
PI_rmse_upr
PI_rmse_lwr
...
kf_rmse_w <- kFoldCV(K = 6,
        reps      = 10,
        randSeed  = 11,
        data = wa_dat_scale_wet,
        model = modw
        )
kf_rmse_u <- kFoldCV(K = 6,
        reps      = 10,
        randSeed  = 11,
        data = wa_dat_scale_upl,
        model = modu
        )
...
lmerTest::ranova(mod7d1)
#lmerTest::difflsmeans(mod7d1, test.effs = "LITH", which = "LITH")
emmeans::emmeans(mod7d1, list(pairwise ~ LITH), adjust = "tukey")

lmerTest::ranova(modw)
#lmerTest::difflsmeans(modw, test.effs = "LITH", which = "LITH")
#emmeans::emmeans(modw, list(pairwise ~ LITH), adjust = "tukey")

lmerTest::ranova(modu)
#lmerTest::difflsmeans(modu, test.effs = "GEO", which = "GEO")
emmeans::emmeans(modu, list(pairwise ~ LITH), adjust = "tukey")
```
...
gammod <- gam((SOC_stock_spline) ~ LITH + CHM + 
                HLI + s(MAP*lower_depth) + s(WIP) +
               s(sample_ID, bs = "re"), 
              family = Gamma(link = "log"), 
              data = wa_dat_scale)
...
rf_dat <- wa_dat_scale |> select(SOC_stock_spline, WIP, MAP, LITH,
                                 MAT, CHM, HLI, EVI, lower_depth,
                                 dev_1000, dev_300, dev_50, grad_1000, grad_300, grad_50) |> 
  mutate(LITH = as.character(LITH))
rf_dat_w <- wa_dat_scale |> select(SOC_stock_spline, WIP, MAP, LITH,
                                 MAT, CHM, HLI,  EVI, lower_depth,
                                 dev_1000, dev_300, dev_50, grad_1000, grad_300, grad_50) |> 
  mutate(LITH = as.character(LITH)) |> 
  filter(WIP >= (0.5 - attr(wa_dat_scale$WIP, "scaled:center"))/attr(wa_dat_scale$WIP, "scaled:scale"))
rf_dat_u <- wa_dat_scale |> select(SOC_stock_spline, WIP, MAP, LITH,
                                 MAT, CHM, HLI,  EVI, lower_depth,
                                 dev_1000, dev_300, dev_50, grad_1000, grad_300, grad_50) |> 
  mutate(LITH = as.character(LITH)) |> 
  filter(WIP < (0.5 - attr(wa_dat_scale$WIP, "scaled:center"))/attr(wa_dat_scale$WIP, "scaled:scale"))
...
train$LITH <- factor(as.character(train$LITH))

test <- rf_dat[-train.index,]
test$LITH<- factor(as.character(test$LITH))

rf_dat$LITH <- factor(as.character(rf_dat$LITH))
rf_dat_w$LITH <- factor(as.character(rf_dat_w$LITH))
rf_dat_u$LITH <- factor(as.character(rf_dat_u$LITH))

write.csv(rf_dat, "SOIL CARBON/All_WA/data/dataframes/RFM_All_data.csv")
write.csv(test, "SOIL CARBON/All_WA/data/dataframes/RFM_Test_data.csv")
write.csv(train, "SOIL CARBON/All_WA/data/dataframes/RFM_Train_data.csv")
```


```{r}
set.seed(11)
tuneRF(x = train[, 2:ncol(train)], y = (train$SOC_stock_spline), mtryStart = 3, ntreeTry = 500, stepFactor = 1.5, improve = 0.001)
```
...
mod_tc <- caret::train((SOC_stock_spline) ~ .,
                        method = "rf",
                       metric = "RMSE",
                         data = train,
...
rf.wet.test <- predict(rf_model, newdata = test.wet)
rf.upl.test <- predict(rf_model, newdata = test.upl)
rf.all <- predict(rf_model, newdata = rf_dat) # has other predictors
...
errdf <- data.frame("LandscapeClass" = c("Full", "Wetland", "Upland"), 
    "RFM.R2_full" = c(r.sq(y.fitted = rf.all, y = (rf_dat$SOC_stock_spline)),
      r.sq(y.fitted = rf.wet, y = (rf_dat_w$SOC_stock_spline)),
      r.sq(y.fitted = rf.upl, y = (rf_dat_u$SOC_stock_spline))),
    "RFM.R2_test" = c(r.sq(y.fitted = rf.test, y = (test$SOC_stock_spline)),
                      r.sq(y.fitted = rf.wet.test, y = (test.wet$SOC_stock_spline)),
                      r.sq(y.fitted = rf.upl.test, y = (test.upl$SOC_stock_spline))),
    "RFM.RMSE_test" = c(caret::postResample(pred = rf.test, obs = (test$SOC_stock_spline))[[1]],
                      caret::postResample(pred = rf.wet.test, obs = (test.wet$SOC_stock_spline))[[1]],
                      caret::postResample(pred = rf.upl.test, obs = (test.upl$SOC_stock_spline))[[1]]))
errdf

test.scale <- wa_dat_scale[-train.index,]
test.wetscale <- wa_dat_scale[-train.index,] |> 
  filter(WIP >= (0.5 - attr(wa_dat_scale$WIP, "scaled:center"))/attr(wa_dat_scale$WIP, "scaled:scale"))
test.uplscale <-wa_dat_scale[-train.index,] |> 
  filter(WIP <( 0.5 - attr(wa_dat_scale$WIP, "scaled:center"))/attr(wa_dat_scale$WIP, "scaled:scale"))


readr::write_csv(errdf, file = "SOIL CARBON/All_WA/data/dataframes/RFM_ErrorDF.csv")
write.csv(test.scale, file = "SOIL CARBON/All_WA/data/dataframes/RFM_testData_scalepreds.csv")
write.csv(test.wetscale, file = "SOIL CARBON/All_WA/data/dataframes/RFM_testwetData_scalepreds.csv")
write.csv(test.uplscale, file = "SOIL CARBON/All_WA/data/dataframes/RFM_testuplData_scalepreds.csv")
...
MAP_pdp<- data.frame(randomForest::partialPlot(rf_model, test, MAP, plot = FALSE)) |> 
  ...
  theme(panel.background = element_blank(),
          panel.grid.major = element_line(colour = "grey80"))
lower_pdp <- data.frame(randomForest::partialPlot(rf_model, test, grad_300, plot = FALSE)) |> 
    ggplot() +
        geom_line(aes(x = x, y = y))+
    xlab("Slope 300m") +
    ylab(expression('Model Prediction Effect')) +
...
sv_dependence(shapviz(ks), v = c("WIP", "MAP", "lower_depth", "grad_50"), color_var = "EVI")
...
# and the data to be used for analyzing the model. The interpretation methods in the iml package need the machine
# learning model to be wrapped in a Predictor object.

iml_mod <- Predictor$new(rf_model, data = rf_dat[-which(names(rf_dat) == "SOC_stock_spline")])
ia_depth <- Interaction$new(iml_mod, feature = "lower_depth")
ia_map <- Interaction$new(iml_mod, feature = "MAP")
ia_wip <- Interaction$new(iml_mod, feature = "WIP")
ia_grad <- Interaction$new(iml_mod, feature = "grad_300")
...
```{r}
ia_depth$results |> data.frame() |> mutate(.feature = factor(.feature)) |>
...
ia_map$results <- ia_map$results |> mutate(.feature = factor(.feature)) |>
  mutate(.feature = fct_reorder(.feature, desc(.interaction)))
ia_wip$results <- ia_wip$results |> mutate(.feature = factor(.feature)) |>
  mutate(.feature = fct_reorder(.feature, desc(.interaction)))
ia_grad$results <- ia_grad$results |> mutate(.feature = factor(.feature)) |>
  mutate(.feature = fct_reorder(.feature, desc(.interaction)))
...
ia_grad_plot <- plot(ia_grad)

save(ia_depth_plot, file = "SOIL CARBON/All_WA/writing/Figures/RFM_Interact_Depth_plot.RData")
save(ia_map_plot, file = "SOIL CARBON/All_WA/writing/Figures/RFM_Interact_MAP_plot.RData")
save(ia_wip_plot, file = "SOIL CARBON/All_WA/writing/Figures/RFM_Interact_WIP_plot.RData")
save(ia_grad_plot, file = "SOIL CARBON/All_WA/writing/Figures/RFM_Interact_grad300_plot.RData")
```
