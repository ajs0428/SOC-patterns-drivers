...
dat_aug <- tw |>
  mutate(
    pred_marg = 10**(predict(modlab_dredge_1, re.form = NA)),    # fixed effects only
    pred_cond = 10**(predict(modlab_dredge_1, re.form = NULL) )   # fixed + random
  )

r2_subset_wet <- dat_aug |>
  filter(WIP_filter >= 0.5) |>
  summarise(
    r2_marginal    = 1 - sum((carbon_perc - pred_marg)^2) / sum((carbon_perc - mean(carbon_perc))^2),
    r2_conditional = 1 - sum((carbon_perc - pred_cond)^2) / sum((carbon_perc - mean(carbon_perc))^2)
  )
...
mod_errdf_lab <- data.frame("LandscapeClass" = c("Full", "Wetland", "Upland"),
                            "LMM.LogMargR2" = c(mumin_R2m_lab, r2_subset_wet$r2_marginal, r2_subset_upl$r2_marginal),
                            "LMM.LogCondR2" = c(mumin_R2c_lab, r2_subset_wet$r2_conditional, r2_subset_upl$r2_conditional),
