...
```{r}
pnw_dat <- read.csv(here::here("data/dataframes/PNW_data_MLR3_RFM_LMM.csv")) |>
  dplyr::select(-site, -X, ) |>
  dplyr::mutate(across(where(is.character), as.factor))
train <- read.csv(here::here("data/dataframes/Train_data.csv")) |> 
  dplyr::select(-site, -X, ) |> dplyr::mutate(across(where(is.character), as.factor))
test <- read.csv(here::here("data/dataframes/Test_data.csv")) |> 
  dplyr::select(-site, -X, ) |> dplyr::mutate(across(where(is.character), as.factor)) |> 
  mutate(geomorphons = factor(geomorphons, levels = levels(train$geomorphons)))
test.wet <- read.csv(here::here("data/dataframes/RFM_Test_Wet_data.csv")) |> 
  dplyr::select(-site, -X, ) |> dplyr::mutate(across(where(is.character), as.factor)) |> 
  mutate(geomorphons = factor(geomorphons, levels = levels(train$geomorphons)),
         LITH = factor(LITH, levels = levels(train$LITH)))
test.upl <- read.csv(here::here("data/dataframes/RFM_Test_Upl_data.csv")) |> 
  dplyr::select(-site, -X, ) |> dplyr::mutate(across(where(is.character), as.factor)) |> 
  mutate(geomorphons = factor(geomorphons, levels = levels(train$geomorphons)),
         LITH = factor(LITH, levels = levels(train$LITH)))
...
# RF_task_full <- mlr3::as_task_regr(pnw_dat, target = "SOC_stock_spline", id = "rfm")
# RF_task_full$set_col_roles("sample_ID", "group")
# RF_task_full$feature_names
# to_remove <- RF_task_full$feature_names[str_detect(RF_task_full$feature_names, "_1000|_300|_50|nlcd")]
# RF_task_full$select(dplyr::setdiff(RF_task_full$feature_names, to_remove))
# RF_task_full$feature_names
```
...
saveRDS(instance, file = here("analysis/models/MLR3_RecursiveFeat_Stocks.rds"))
```



```{r}
instance <- readRDS(here::here("analysis/models/MLR3_RecursiveFeat_Stocks.rds"))
...
saveRDS(tune_instance, file = here("analysis/models/MLR3_RFM_Ranger_tune_instance.rds"))
```
...
tune_instance <- readRDS(file = here("analysis/models/MLR3_RFM_Ranger_tune_instance.rds"))
RF_lrn_tuned = lrn("regr.ranger")
RF_lrn_tuned$param_set$values = tune_instance$result_learner_param_vals[[1]]
RF_lrn_tuned$param_set$values$num.threads <- 1    # <- threads fixed
RF_lrn_tuned$param_set$values$keep.inbag <- TRUE  # <- REQUIRED for ranger quantile regression; set AFTER the param-set overwrite so it survives
RF_lrn_tuned$predict_type      = "quantiles"
RF_lrn_tuned$quantiles         <- c(0.025, 0.5, 0.975)  # in current mlr3learners these are R6 fields, NOT lrn() args
RF_lrn_tuned$quantile_response <- 0.5
set.seed(11)                                      # <- seed right before the fit
RF_lrn_tuned$train(RF_task)$model

saveRDS(RF_lrn_tuned, here("analysis/models/MLR3_RFM_Ranger_tuned.rds")) # This is the final model


# RF_lrn_full_tuned = lrn("regr.ranger", 
#                         quantile_response = 0.5,
#               quantiles = c(0.025, 0.5, 0.975))
# RF_lrn_full_tuned$predict_type = "quantiles"
# RF_lrn_full_tuned$param_set$values = tune_instance$result_learner_param_vals[[1]]
# RF_lrn_full_tuned$train(RF_task_full)$model
# 
# RF_lrn_tuned_full <- RF_lrn_full_tuned$train(RF_task_full)
# saveRDS(RF_lrn_tuned_full, here("analysis/models/MLR3_RFM_Ranger_tuned_fulldata.rds")) # This is the final model

```

```{r}
RF_lrn_tuned <- readRDS(here::here("analysis/models/MLR3_RFM_Ranger_tuned.rds"))
RF_lrn_tuned$model
```
...
RF_CV <- rsmp("repeated_cv", repeats = 10, folds = 10)

RF_rr <- mlr3::resample(RF_task, RF_lrn_tuned, RF_CV)
saveRDS(RF_rr, file = here("analysis/models/MLR3_RFM_Ranger_resampling.rds"))
```


```{r}
set.seed(11)
RF_rr <- readRDS(file = here("analysis/models/MLR3_RFM_Ranger_resampling.rds"))
...
write_csv(RF_rr_acc, file = here("data/dataframes/MLR3_RFM_Ranger_tuned_CV_results.csv"))
```


```{r}
RF_rr_acc <- read.csv(here::here("data/dataframes/MLR3_RFM_Ranger_tuned_CV_results.csv"))
...
write_csv(tdf, here("data/dataframes/PNW_SOC_RF_test_pred_response.csv"))
ggplot() +
...
ks <- kernelshap(object = RF_lrn_tuned$model$model, X = rtbx, bg_X = train_select, parallel = T )
ks

ks_wet <- kernelshap(object = RF_lrn_tuned$model$model, X = rtbx_wet, bg_X = train_select, parallel = T)
ks_wet

ks_upl <- kernelshap(object = RF_lrn_tuned$model$model, X = rtbx_upl, bg_X = train_select, parallel = T)
ks_upl

saveRDS(ks, file = here("analysis/models/MLR3_RFM_Ranger_KernelShap.rds"))
saveRDS(ks_wet, 
        file = here("analysis/models/MLR3_RFM_Ranger_KernelShap_Wet.rds"))
saveRDS(ks_upl, 
        file = here("analysis/models/MLR3_RFM_Ranger_KernelShap_Upl.rds"))

# ks_full <- kernelshap(object = RF_lrn_full_tuned$model$model, X = pnw_dat_select, bg_X = train_select, parallel = T)
# 
# saveRDS(ks_full, 
#         file = here("analysis/models/MLR3_RFM_Ranger_KernelShap_Full.rds"))
```
...
ks_upl_vars <- ks_upl$S |> as.data.frame() |> dplyr::summarise(across(everything(), ~sum(abs(.x)))) |> pivot_longer(everything(), names_to = "column", values_to = "abs_sum") %>%
    arrange(desc(abs_sum)) %>%
    slice_head(n = 5)
ks_full_vars <- ks_full$S |> as.data.frame() |> dplyr::summarise(across(everything(), ~sum(abs(.x)))) |> pivot_longer(everything(), names_to = "column", values_to = "abs_sum") %>%
    arrange(desc(abs_sum)) %>%
    slice_head(n = 5)
...
predictor <- iml::Predictor$new(RF_lrn_tuned,
                                data = RF_task_feats, 
                                y = RF_task_target)
saveRDS(predictor, here::here("analysis/models/MLR3_Predictor.rds"))
...
saveRDS(ia_1, 
        file = here::here("writing/Figures/MLR3_RFM_Interact_1_plot.rds"))
saveRDS(ia_2, 
        file = here::here("writing/Figures/MLR3_RFM_Ranger_Interact_2_plot.rds"))
saveRDS(ia_3, 
        file = here::here("writing/Figures/MLR3_RFM_Ranger_Interact_3_plot.rds"))
saveRDS(ia_4, 
        file = here::here("writing/Figures/MLR3_RFM_Ranger_Interact_4_plot.rds"))
saveRDS(ia_5, 
        file = here::here("writing/Figures/MLR3_RFM_Ranger_Interact_5_plot.rds"))
```
...
saveRDS(ia_1_plot, file = here::here("writing/Figures/MLR3_RFM_Interact_1_plot.rds"))
saveRDS(ia_2_plot, file = here::here("writing/Figures/MLR3_RFM_Interact_2_plot.rds"))
saveRDS(ia_3_plot, file = here::here("writing/Figures/MLR3_RFM_Interact_3_plot.rds"))
saveRDS(ia_4_plot, file = here::here("writing/Figures/MLR3_RFM_Interact_4_plot.rds"))
saveRDS(ia_5_plot, file = here::here("writing/Figures/MLR3_RFM_Interact_5_plot.rds"))
```


```{r}
m <- RF_lrn_tuned$model 
p <- m$model$predictions
t <- RF_task$truth()
w <- RF_task$data(cols = "WIP")$WIP

oob_tbl <- tibble(truth = t, pred = p, wip = w)

oob_metrics_subset_wet <- oob_tbl |>
  filter(wip >= 0.5) |>
  summarise(
...
oob_metrics_subset_upl <- oob_tbl |>
  filter(wip < 0.5) |>
  summarise(
    rmse = sqrt(mean((truth - pred)^2)),
    r2   = 1 - sum((truth - pred)^2) / sum((truth - mean(truth))^2)
...
oob_err_wetupl <- bind_rows(oob_metrics_subset_wet, oob_metrics_subset_upl) |> 
...
qrf_errdf <- data.frame("LandscapeClass" = c("Full", "Wetland", "Upland"), 
    "RFM.R2_OOB" = c(
      RF_lrn_tuned$model$model$r.squared,
      oob_err_wetupl$r2[[1]],
      oob_err_wetupl$r2[[2]]
      ),
    "RFM.RMSE_OOB" = c(
      sqrt(RF_lrn_tuned$model$model$prediction.error),
      oob_err_wetupl$rmse[[1]],
      oob_err_wetupl$rmse[[2]]
      ),
...
readr::write_csv(qrf_errdf, file = here::here("data/dataframes/MLR3_RFM_ErrorDF.csv"))
```
