...
modT7 <- lmer(log10(SOC_stock_spline) ~ 
                CHM + HLI + WIP + PET_MAP + (LITH) +
                lower_depth + (lower_depth|sample_ID),
                #family = Gamma(link = "log"),
                data = train_scale, REML = F
              )
#Interaction with HLI and PET_MAP
modT7h <- lmer(log10(SOC_stock_spline) ~ 
                   WIP+HLI*PET_MAP + (LITH)  + 
                lower_depth + (lower_depth|sample_ID),
                #family = Gamma(link = "log"),
                data = train_scale, REML = F
              )
...
summary(mod_dredge_1)
anova(mod_dredge_1)
MuMIn::r.squaredGLMM(mod_dredge_1)
r2glmm::r2beta(mod_dredge_1, method = "nsj")

anova(mod1, mod_dredge_1)

saveRDS(mod_dredge_1, file = "SOIL CARBON/All_WA/analysis/models/PNW_Model_Dredge_Spline.rds")
```
...
plot(mod_dredge_1)
plot(mod_dredge_1, resid(.) ~ fitted(.)|sample_ID, abline = 0, asp = 1)

mod_resid_fit <- data.frame("sample_ID" = train_scale$sample_ID, 
                            "resid" = resid(mod_dredge_1), 
                            "fitted" = fitted(mod_dredge_1))
...
round(r.sq(y = (test_scale$SOC_stock_spline), 
           y.fitted = 10**predict(mod_dredge_1, test_scale, allow.new.levels = TRUE)), 3)
round(r.sq(y = (test_wet_scale$SOC_stock_spline), 
           y.fitted = 10**predict(mod_dredge_1, test_wet_scale, allow.new.levels = TRUE)), 3)
round(r.sq(y = (test_upl_scale$SOC_stock_spline), 
           y.fitted = 10**predict(mod_dredge_1, test_upl_scale, allow.new.levels = TRUE)), 3)

caret::RMSE(pred = 10**predict(mod_dredge_1, test_scale, allow.new.levels = TRUE), 
            obs = test_scale$SOC_stock_spline)
caret::RMSE(pred = 10**predict(mod_dredge_1, test_wet_scale, allow.new.levels = TRUE), 
            obs = test_wet_scale$SOC_stock_spline)
caret::RMSE(pred = 10**predict(mod_dredge_1, test_upl_scale, allow.new.levels = TRUE), 
            obs = test_upl_scale$SOC_stock_spline)
r2glmm::r2beta(mod_dredge_1, method = "nsj")[1,]

ggplot()+
  geom_point(aes(y = test_scale$SOC_stock_spline,
                 x = 10**predict(mod_dredge_1, test_scale, allow.new.levels = TRUE),
...
qqnorm(resid(mod_dredge_1))
qqline(resid(mod_dredge_1))



lev <- hat(model.matrix(mod_dredge_1))
cd <- cooks.distance(mod_dredge_1)
plot(resid(mod_dredge_1) ~ lev, xlab = "leverage", ylab = "Residuals LMM Full")
rec <- recordPlot()
...
hoh_st <- st_read("SOIL CARBON/All_WA/data/points/hoh_pts_2855.gpkg") |> select(names(hlef_st))
mas_st <- st_read("SOIL CARBON/All_WA/data/points/mas_pts_2856.gpkg") |> select(names(hlef_st))
col_st <- st_read("SOIL CARBON/All_WA/data/points/col_pts_2855.gpkg") |> select(names(hlef_st))
...
resids <- resid(mod_dredge_1)

resids_hlef <- resid(mod_dredge_1)[pnw_st$site == "HLEF"]
resids_hoh <- resid(mod_dredge_1)[pnw_st$site == "Hoh"]
resids_mas <- resid(mod_dredge_1)[pnw_st$site == "Mas"]
resids_col <- resid(mod_dredge_1)[pnw_st$site == "Col"]

vario_all <- geoR::variog(coords = coords, data = resids, max.dist = 400)
...
The model $R^2$ for `mod7` is `r round(r.sq(log10(wa_dat_scale$SOC_stock_spline), fitted(mod_dredge_1)), 3)`
...
ci_anova_df <- ci_anova_tab_func(model = mod_dredge_1, data = train_scale)

ci_anova_df


readr::write_csv(ci_anova_df, "SOIL CARBON/All_WA/data/dataframes/anova_mod_dredge_1_confint95.csv")
...
PI_trans <- PI_func(model = mod_dredge_1, data = test_scale)
PI_trans_wet <- PI_func(model = mod_dredge_1, data = test_wet_scale)
PI_trans_upl <- PI_func(model = mod_dredge_1, data = test_upl_scale)
...
r2_po <- round(r.sq(y = (test_scale$SOC_stock_spline), 
                         y.fitted =  10**predict(mod_dredge_1, 
...
rmse_po <- caret::RMSE(pred = 10**predict(mod_dredge_1, 
                                          test_scale, allow.new.levels = TRUE), 
                       obs = test_scale$SOC_stock_spline)
rmse_po_w <- caret::RMSE(pred = 10**predict(mod_dredge_1, 
                                            test_wet_scale, allow.new.levels = TRUE), 
                         obs = test_wet_scale$SOC_stock_spline)
rmse_po_u <- caret::RMSE(pred = 10**predict(mod_dredge_1, 
                                            test_upl_scale, allow.new.levels = TRUE), 
                         obs = test_upl_scale$SOC_stock_spline)

plot(x = test_scale$SOC_stock_spline, y = 10**predict(mod_dredge_1, test_scale, re.form = NA,
                                                          allow.new.levels = T)) + abline(0,1)

mumin_R2m <- MuMIn::r.squaredGLMM(mod_dredge_1)[[1]]
mumin_R2c <- MuMIn::r.squaredGLMM(mod_dredge_1)[[2]]
