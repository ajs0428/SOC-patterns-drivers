...
train_scale <- read.csv(
          "data/dataframes/PNW_Spline_Stocks_Train_Scale.csv" |> here())
train_wet_scale<- read.csv(
          "data/dataframes/PNW_Spline_Stocks_Train_Wet_Scale.csv" |> here())
train_upl_scale<- read.csv(
          "data/dataframes/PNW_Spline_Stocks_Train_Upl_Scale.csv" |> here())
test_scale<- read.csv(
          "data/dataframes/PNW_Spline_Stocks_Test_Scale.csv" |> here())
test_wet_scale<- read.csv(
          "data/dataframes/PNW_Spline_Stocks_Test_Wet_Scale.csv" |> here())
test_upl_scale<- read.csv(
          "data/dataframes/PNW_Spline_Stocks_Test_Upl_Scale.csv" |> here())
...
mod_dredge <- MuMIn::dredge(gmod, rank = "AIC", #extra = "R^2", 
                            m.lim = c(1,10), 
...
anova(mod_dredge_2, mod_dredge_3)
```
...
saveRDS(best_dredge_model, file = "analysis/models/PNW_Model_Dredge_Spline.rds" |> here())
```
...
save(model_resid_fit_plot, file = "writing/Figures/Supplemental/model_resid_fit_plot.Rdata" |> here())
```
...
best_dredge_model_R2list <- c(
  "lmmMargR2" = MuMIn::r.squaredGLMM(best_dredge_model)[1],
  "lmmCondR2" = MuMIn::r.squaredGLMM(best_dredge_model)[2],
  "lmmTestR2" = round(r.sq(y = (test_scale$SOC_stock_spline), 
           y.fitted = 10**predict(best_dredge_model, test_scale, allow.new.levels = TRUE)), 3),
  "lmmTestR2wet" = round(r.sq(y = (test_wet_scale$SOC_stock_spline), 
           y.fitted = 10**predict(best_dredge_model, test_wet_scale, allow.new.levels = TRUE)), 3),
  "lmmTestR2upl" = round(r.sq(y = (test_upl_scale$SOC_stock_spline), 
           y.fitted = 10**predict(best_dredge_model, test_upl_scale, allow.new.levels = TRUE)), 3),

  "lmmTestRMSE" = caret::RMSE(pred = 10**predict(best_dredge_model, test_scale, allow.new.levels = TRUE), 
            obs = test_scale$SOC_stock_spline),
  "lmmTestRMSEwet" = caret::RMSE(pred = 10**predict(best_dredge_model, test_wet_scale, allow.new.levels = TRUE), 
            obs = test_wet_scale$SOC_stock_spline),
  "lmmTestRMSEupl" = caret::RMSE(pred = 10**predict(best_dredge_model, test_upl_scale, allow.new.levels = TRUE), 
            obs = test_upl_scale$SOC_stock_spline)
  )
best_dredge_model_R2list
saveRDS(best_dredge_model_R2list, file = here("writing/in_text_values/best_dredge_model_R2list.rds") )
...
png(filename = "writing/Figures/Supplemental/qq_lev_full.png" |> here(), width = 12, height = 8, units = "in", res = 300)
par(mfcol = c(2,2))
...
hlef_st <- st_read("data/points/hlef_pts_6394.gpkg" |> here()) 
hoh_st <- st_read("data/points/hoh_pts_2855.gpkg" |> here()) |> dplyr::select(names(hlef_st))
mas_st <- st_read("data/points/mas_pts_2856.gpkg" |> here()) |> dplyr::select(names(hlef_st))
col_st <- st_read("data/points/col_pts_2855.gpkg" |> here()) |> dplyr::select(names(hlef_st))
...
# #write.csv(wa_dat_scale, "data/dataframes/PNW_SOC_ExtractPreds_SFcoords.csv" |> here())
# 
# 
...
(ggplot(vario_h_df, aes(x = distance_km, y = semivariance)) +
  ...
  ylim(0, 1.2*max(vario_h_df$semivariance)))/
(ggplot(vario_m_df, aes(x = distance_km, y = semivariance)) +
  geom_point()+ 
  xlab("Distance MRW (km)") +
  ylim(0, 1.2*max(vario_m_df$semivariance)))/
(ggplot(vario_c_df, aes(x = distance_km, y = semivariance)) +
  geom_point()+ 
  xlab("Distance CVW (km)") +
  ylim(0, 1.2*max(vario_c_df$semivariance))) + plot_layout(axes = "collect")

save(semivariograms, file = "writing/Figures/Supplemental/Semivariograms.RData" |> here())
```
...
readr::write_csv(ci_anova_df, "data/dataframes/anova_best_dredge_model_confint95.csv" |> here())
...
saveRDS(PI_trans, file = "data/dataframes/LMM_PredictInterval_RMSE_Stocks.rds" |> here())
saveRDS(PI_trans_wet, 
        file = "data/dataframes/LMM_Wet_PredictInterval_RMSE_Stocks.rds" |> here())
saveRDS(PI_trans_upl, 
        file = "data/dataframes/LMM_PredictInterval_Upl_RMSE_Stocks.rds" |> here())
...
kf_df
readr::write_csv(kf_df, file = "data/dataframes/KFold_RMSE_Depths_Dataframe.csv" |> here())

```


```{r}
tw_noscale <- read.csv(here("data/dataframes/Train_data.csv")) |> 
  dplyr::select(-X) |> dplyr::mutate(across(where(is.character), as.factor))
tw <- read.csv(
          "data/dataframes/PNW_Spline_Stocks_Train_Scale.csv" |> here())
tw$WIP_filter <- tw_noscale$WIP
dat_aug <- tw |>
  mutate(
    pred_marg = 10**(predict(best_dredge_model, re.form = NA)),    # fixed effects only
    pred_cond = 10**(predict(best_dredge_model, re.form = NULL) )   # fixed + random
  )

r2_subset_wet <- dat_aug |>
  filter(WIP_filter >= 0.5) |>
  summarise(
    r2_marginal    = 1 - sum((SOC_stock_spline - pred_marg)^2) / sum((SOC_stock_spline - mean(SOC_stock_spline))^2),
    r2_conditional = 1 - sum((SOC_stock_spline - pred_cond)^2) / sum((SOC_stock_spline - mean(SOC_stock_spline))^2)
  )

r2_subset_upl <- dat_aug |>
  filter(WIP_filter < 0.5) |>
  summarise(
    r2_marginal    = 1 - sum((SOC_stock_spline - pred_marg)^2) / sum((SOC_stock_spline - mean(SOC_stock_spline))^2),
    r2_conditional = 1 - sum((SOC_stock_spline - pred_cond)^2) / sum((SOC_stock_spline - mean(SOC_stock_spline))^2)
...
LMM_testpred <- data.frame(truth = test_scale$SOC_stock_spline,
                             predict = 10**predict(best_dredge_model, test_scale, re.form = NA,
                                                          allow.new.levels = T))
readr::write_csv(LMM_testpred,
                 here("data/dataframes/PNW_SOC_LMM_test_pred_response.csv")) 

mod_errdf <- data.frame("LandscapeClass" = c("Full", "Wetland", "Upland"),
                                "LMM.LogMargR2" = c(mumin_R2m, r2_subset_wet$r2_marginal, r2_subset_wet$r2_conditional),
                                "LMM.LogCondR2" = c(mumin_R2c, r2_subset_upl$r2_marginal, r2_subset_upl$r2_conditional),
...
readr::write_csv(mod_errdf, file = "data/dataframes/PNW_Stocks_model_fit.csv" |> here())
```
