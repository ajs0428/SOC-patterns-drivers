...
```{r}
#| eval: false
hoh_stack_depths_mask_scale <- rast("SOIL CARBON/All_WA/data/Rasters/PredictorStacks/hoh_stack_depths_water_urban_mask_scale.tif")
mas_stack_depths_mask_scale <- rast("SOIL CARBON/All_WA/data/Rasters/PredictorStacks/mas_stack_depths_water_urban_mask_scale.tif")
col_stack_depths_mask_scal <- rast( "SOIL CARBON/All_WA/data/Rasters/PredictorStacks/col_stack_depths_water_urban_mask_scale.tif")

hoh_stack_pedons_mask_scale <- rast( "SOIL CARBON/All_WA/data/Rasters/PredictorStacks/hoh_stack_pedons_water_urban_mask_scale.tif")
mas_stack_pedons_mask_scale <- rast( "SOIL CARBON/All_WA/data/Rasters/PredictorStacks/mas_stack_pedons_water_urban_mask_scale.tif")
col_stack_pedons_mask_scale <- rast( "SOIL CARBON/All_WA/data/Rasters/PredictorStacks/col_stack_pedons_water_urban_mask_scale.tif")

hoh_stack_depths_mask_scale$LITH <- subst(hoh_stack_depths_mask_scale$LITH, from = c(1,2,3,4), c("glacial outwash", "glacial till and drift",
                                                                                     "sedimentary", "unconsolidated"))
mas_stack_depths_mask_scale$LITH <- subst(mas_stack_depths_mask_scale$LITH, from = c(1,2,3,4,5), c("glacial outwash", "glacial till and drift",
                                                                                       "igneous", "sedimentary", "unconsolidated"))
col_stack_depths_mask_scal$LITH <- subst(col_stack_depths_mask_scal$LITH, from = c(1,2,3,4,5,6), c("glacial outwash", "glacial till and drift",
                                                                                         "igneous", "metamorphic", "sedimentary", "unconsolidated")) 

hoh_stack_pedons_mask_scale$LITH <- subst(hoh_stack_pedons_mask_scale$LITH, from = c(1,2,3,4), c("glacial outwash", "glacial till and drift",
                                                                                     "sedimentary", "unconsolidated"))
mas_stack_pedons_mask_scale$LITH <- subst(mas_stack_pedons_mask_scale$LITH, from = c(1,2,3,4,5), c("glacial outwash", "glacial till and drift",
                                                                                       "igneous", "sedimentary", "unconsolidated"))
col_stack_pedons_mask_scale$LITH <- subst(col_stack_pedons_mask_scale$LITH, from = c(1,2,3,4,5,6), c("glacial outwash", "glacial till and drift",
                                                                                         "igneous", "metamorphic", "sedimentary", "unconsolidated"))
```
...
SOC_pred_func_depths <- function(stack, depths, model, site, path){
  const_df_15 <- data.frame(site = site, 
      ...
      } else if (str_detect(deparse(substitute(model)), "depths") == TRUE){
        print("Linear Model")
        for(i in 1:depths){
...
SOC_pred_func_pedon <- function(stack, model, path, site){
  terra::predict(stack, model, #se.fit = TRUE,
                     na.rm = TRUE, 
                     type = "response",
...
test_pred_pedon <- SOC_pred_func_pedon(stack = test, model = GLM, path = SOC_pred_path, site = "MAS")

plot(test_pred_depths)
plot(rast("SOIL CARBON/All_WA/data/Rasters/SOC_Predictions/"))
...
```{r}
#| eval: false
SOC_pred_func_pedon(stack = hoh_stack_pedons_mask_scale, model = GLM, path = SOC_pred_path, 
                    site = "hoh")
SOC_pred_func_pedon(stack = mas_stack_pedons_mask_scale, model = GLM, path = SOC_pred_path,
                    site = "mas")
SOC_pred_func_pedon(stack = col_stack_pedons_mask_scale, model = GLM, path = SOC_pred_path,
                    site = "col")
...
- may not need to do this if already masked in geospatial prep script
```{r}
#| eval: false
...
C_map_wet_fractions <- function(C_map, WIP){
    ...
    return(data.frame("Name" = c(name, paste0(name, "_wet"), paste0(name, "_upl")), 
                      ...
                      "Perc10thSOC_Mgha" = c(C_quant_all["10%"], C_quant_wet["10%"], C_quant_upl["10%"]),
                      "Perc90thSOC_Mgha" = c(C_quant_all["90%"], C_quant_wet["90%"], C_quant_upl["90%"]),
...
SOC_df_form <- SOC_df |> 
  ...
  rename("Study Area" = Name,
           ...
           "10th Percentile SOC Stock (Mg ha-1)" = Perc10thSOC_Mgha,
           "90th Percentile SOC Stock (Mg ha-1)" = Perc90thSOC_Mgha,
...
```{r}
t <- terra::merge(soc_list$hoh_pedon, soc_list$mas_pedon, soc_list$col_pedon)
quantile(values(t), probs = c(10, 90), na.rm = T)
```
