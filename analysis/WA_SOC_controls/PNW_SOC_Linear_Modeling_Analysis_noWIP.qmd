...
mod_dredge <- dredge(gmod, rank = "AIC", m.lim = c(1,10), 
                     subset =  (xor(MAP, PET_MAP) && !(MAP&&PET_MAP)),
                    fixed = c("lower_depth", "geomorphons", "LITH"))
...
saveRDS(best_dredge_model, file = "analysis/models/PNW_Model_Dredge_Spline_noWIP.rds" |> here())
```
...
save(model_resid_fit_plot, file = "writing/Figures/Supplemental/model_resid_fit_plot_noWIP.rdata" |> here())
```
...
saveRDS(best_dredge_model_R2list, file = here("writing/in_text_values/best_dredge_model_R2list_noWIP.rds"))
...
LMM_testprednw <- data.frame(truth = test_scale$SOC_stock_spline,
                             predict = 10**predict(best_dredge_model, test_scale,
                                                   allow.new.levels = TRUE))
readr::write_csv(LMM_testprednw,
                 here("data/dataframes/PNW_SOC_LMM_noWIP_test_pred_response.csv"))
```
...
png(filename = "writing/Figures/Supplemental/qq_lev_full_noWIP.png" |> here(), width = 12, height = 8, units = "in", res = 300)
...
readr::write_csv(ci_anova_df, "data/dataframes/anova_best_dredge_model_confint95_noWIP.csv" |> here())
...
saveRDS(PI_trans, file = "data/dataframes/LMM_PredictInterval_RMSE_StocksnoWIP_noWIP.rds" |> here())
saveRDS(PI_trans_wet, 
        file = "data/dataframes/LMM_Wet_PredictInterval_RMSE_Stocks_noWIP.rds" |> here())
saveRDS(PI_trans_upl, 
        file = "data/dataframes/LMM_PredictInterval_Upl_RMSE_Stocks_noWIP.rds" |> here())
...
kf_df
readr::write_csv(kf_df, file = "data/dataframes/KFold_RMSE_Depths_Dataframe_noWIP.csv" |> here())
...
tw_nw <- read.csv(
          "data/dataframes/PNW_Spline_Stocks_Train_Scale.csv" |> here())
tw_nw$WIP_filter <- tw_noscale$WIP
dat_aug_nw <- tw_nw |>
  mutate(
...
mod_errdf <- data.frame("LandscapeClass" = c("Full", "Wetland", "Upland"),
                                "LMM.LogMargR2" = c(mumin_R2m, r2_subset_wet_nw$r2_marginal, r2_subset_upl_nw$r2_marginal),
                                "LMM.LogCondR2" = c(mumin_R2c, r2_subset_wet_nw$r2_conditional, r2_subset_upl_nw$r2_marginal),
...
readr::write_csv(mod_errdf, file = "data/dataframes/PNW_Stocks_model_fit_noWIP.csv" |> here())
```
```{r}
