...
library(formatR)
knitr::opts_chunk$set(echo = TRUE, fig.align = "center", fig.show = "hold", time_it = TRUE, dpi = 100, tidy.opts = list(width.cutoff = 60), tidy = T, collapse = TRUE)
...
# pak::pak(c("mlr-org/mlr3", "mlr-org/mlr3fselect"))
...
```{r}
pred_path <- here("data/Rasters/PredictorStacks/")
```
...
QRFM_stocks_nw <- readRDS(here("analysis/models/MLR3_RFM_Ranger_tuned_noWIP.rds"))
```
...
```{r}
#| fig-show: hold
hlef_consolidate_lith <- vect(here("data/Vectors/HLEF_lithology_consolidate.gpkg"))
hoh_consolidate_lith <- vect(here("data/Vectors/Hoh_lithology_consolidate.shp"))
mas_consolidate_lith <- vect(here("data/Vectors/Mas_lithology_consolidate.shp"))
col_consolidate_lith <- vect(here("data/Vectors/Col_lithology_consolidate.shp"))
...
```{r}
pnw_lith_lev <- data.frame(id = seq(0, length(levels(pnw_dat$LITH))-1, 1),
                 LITH = levels(pnw_dat$LITH))
hlef_m <- c(0, NA,
            1, 2, 
            2, 3,
            3, 4,
            4, 5) |> matrix(ncol = 2, byrow = TRUE)
hlef_lith_rcl <- terra::classify(hlef_stack$LITH, hlef_m, include.lowest = T)
levels(hlef_lith_rcl) <- pnw_lith_lev
hlef_stack$LITH <- hlef_lith_rcl

hoh_m <- c(0, 0,
            1, 1, 
            2, 4,
           3, 5) |> matrix(ncol = 2, byrow = TRUE)
hoh_lith_rcl <- terra::classify(hoh_stack$LITH, hoh_m, include.lowest = T)
levels(hoh_lith_rcl) <- pnw_lith_lev
hoh_stack$LITH <- hoh_lith_rcl
...
mas_m <- c(0, 0,
            ...
            3, 4,
            4, 5) |> matrix(ncol = 2, byrow = TRUE)
mas_lith_rcl <- terra::classify(mas_stack$LITH, mas_m, include.lowest = T)
levels(mas_lith_rcl) <- pnw_lith_lev
mas_stack$LITH <- mas_lith_rcl
...
col_m <- c(0, 0,
            ...
            3, 3,
            4, 4,
           5, 5) |> matrix(ncol = 2, byrow = TRUE)
col_lith_rcl <- terra::classify(col_stack$LITH, col_m, include.lowest = T)
levels(col_lith_rcl) <- pnw_lith_lev
col_stack$LITH <- col_lith_rcl
```
...
All levels present in each study area

```{r}
pnw_geom_lev <- data.frame(id = seq(1, length(levels(pnw_dat$geomorphons)), 1),
                 geomorphons = levels(pnw_dat$geomorphons))
levels(hlef_stack$geomorphons) <- pnw_geom_lev

levels(hoh_stack$geomorphons) <- pnw_geom_lev

levels(mas_stack$geomorphons) <- pnw_geom_lev

levels(col_stack$geomorphons) <- pnw_geom_lev

plot(hlef_stack$geomorphons)
```
...
#instance <- readRDS("analysis/models/MLR3_RecursiveFeat_Stocks.rds")
features <- (names(QRFM_stocks$model$model$variable.importance)[names(QRFM_stocks$model$model$variable.importance) != "lower_depth"])
features
```
...
writeRaster(hlef_stack_select,
            "data/Rasters/PredictorStacks/HLEF_PredictorStack_Class_RFM_select.tif", 
            overwrite = TRUE)

hoh_stack_select <- hoh_stack |> tidyterra::select(all_of(features))
writeRaster(hoh_stack_select, 
            "data/Rasters/PredictorStacks/Hoh_PredictorStack_Class_RFM_select.tif", 
            overwrite = TRUE)
mas_stack_select <- mas_stack |> tidyterra::select(all_of(features))
writeRaster(mas_stack_select,
            "data/Rasters/PredictorStacks/Mas_PredictorStack_Class_RFM_select.tif", 
            overwrite = TRUE)

col_stack_select <- col_stack |> tidyterra::select(all_of(features))
writeRaster(col_stack_select,
            "data/Rasters/PredictorStacks/Col_PredictorStack_Class_RFM_select.tif", 
            overwrite = TRUE)
...
```{r}
hlef_stack_select <- rast(here("data/Rasters/PredictorStacks/HLEF_PredictorStack_Class_RFM_select.tif"))
hoh_stack_select <- rast(here("data/Rasters/PredictorStacks/Hoh_PredictorStack_Class_RFM_select.tif"))
mas_stack_select <- rast(here("data/Rasters/PredictorStacks/Mas_PredictorStack_Class_RFM_select.tif"))
col_stack_select <- rast(here("data/Rasters/PredictorStacks/Col_PredictorStack_Class_RFM_select.tif"))
...
```{r}
#| eval: false
tilePath <- here("data/Rasters/Tiles")
hlef_tiles <- makeTiles(hlef_stack_select, c(1000, 1000), 
                       filename = file.path(tilePath, "hlef_tile_.tif"))
```


```{r}
soc_path <- here("data/Rasters/SOC_Predictions/SOC_Predictions_Depth/")

soc_pred_func <- function(model, pred_stack, depth_ints) {
  stopifnot(is.vector(depth_ints))
  # stopifnot(numcores < parallel::detectCores())
  # `model` must be the underlying ranger object (e.g. QRFM_stocks$model$model),
  # NOT the mlr3 learner: terra::predict dispatches predict.ranger, and the saved
  # mlr3 learner is unusable under the installed mlr3 (`always_included` col-role).
  # Quantile (median) prediction requires the model trained with keep.inbag = TRUE.
  name <- str_extract(deparse(substitute(pred_stack)), "hlef|hoh|mas|col")
  # predict.ranger returns a ranger.prediction list; pull out $predictions so terra
  # (esp. the parallel cores > 1 path) receives a plain numeric matrix.
  ranger_pred <- function(model, ...) predict(model, ...)$predictions

  for(i in 1:length(depth_ints)) {
    fn <- paste0(soc_path, "PNW_", name,"_", depth_ints[[i]],
                                  "cm_SOC_RFM_2.tif")
    if(!file.exists(fn)){
      predict(pred_stack, model, fun = ranger_pred,
                type = "quantiles", quantiles = 0.5,
                const = data.frame(lower_depth = depth_ints[i]),
                na.rm = TRUE, cpkgs = "ranger",
                filename = paste0(soc_path, "PNW_", name,"_", depth_ints[[i]],
                                  "cm_SOC_RFM_2.tif"),
                overwrite = TRUE)
      gc()
    } else {
      message("already file at: ", fn)
    }
  ...
  # 
  # writeRaster(rast_stack_sum, filename = paste0(soc_path, "PNW_", name,
  #                                               "_SOC_RFM_1m_noWIP.tif"),
  #             overwrite = TRUE)

  #return(rast_stack_sum)
}

# test <- soc_pred_func(QRFM_stocks$model$model, hlef_stack_select, depth_ints = 25)
...
# 
# system.time(
# test <- soc_pred_func(model = QRFM_stocks_nw, pred_stack = r, depth_ints = seq(25, 50, 25),
#                       numcores = 2)
# )
# plot(test)
```
...
```{r}
#| eval: false
#| label: Predict to maps

soc_pred_func(model = QRFM_stocks$model$model,
...
```{r}
hlef_soc_list <- list.files(soc_path, pattern = "hlef", full.names = TRUE) |> str_subset("RFM_2")
hlef_soc <- rast(hlef_soc_list) |> 
  sum() 

hoh_soc_list <- list.files(soc_path, pattern = "hoh", full.names = TRUE) |> str_subset("RFM_2")
hoh_soc <- rast(hoh_soc_list) |> 
  sum() 

mas_soc_list <- list.files(soc_path, pattern = "mas", full.names = TRUE) |> str_subset("RFM_2")
mas_soc <- rast(mas_soc_list) |> 
  sum() 

col_soc_list <- list.files(soc_path, pattern = "col", full.names = TRUE) |> str_subset("RFM_2")
col_soc <- rast(col_soc_list) |> 
  sum() 
plot(hlef_soc)
plot(hoh_soc)
plot(mas_soc)
...
```{r}
#| eval: false
nlcd_cats_hlef <- read.csv(here("data/dataframes/nlcd_cats_hlef.csv")) |> 
  dplyr::select(-X)
nlcd_cats_hoh<- read.csv(here("data/dataframes/nlcd_cats_hoh.csv"))|> 
  dplyr::select(-X)
nlcd_cats_mas<- read.csv(here("data/dataframes/nlcd_cats_mas.csv"))|> 
  dplyr::select(-X)
nlcd_cats_col<- read.csv(here("data/dataframes/nlcd_cats_col.csv"))|> 
  dplyr::select(-X)

hlef_nlcd <- rast(here("data/Rasters/HLEF_nlcd_reclass.tif")) 
levels(hlef_nlcd) <- nlcd_cats_hlef
hoh_nlcd <- rast(here("data/Rasters/Hoh_nlcd_reclass.tif")) 
levels(hoh_nlcd) <- nlcd_cats_hoh
mas_nlcd <- rast(here("data/Rasters/Mashel_nlcd_reclass.tif") )
levels(mas_nlcd) <- nlcd_cats_mas
col_nlcd <- rast(here("data/Rasters/Colville_nlcd_reclass.tif")) 
...
water_urban_masking <- function(rast_to_mask, mndwi, urban) {
  ...
  writeRaster(masked_soc,  filename = paste0(soc_path, deparse(substitute(rast_to_mask)), "_water_urban_mask", "no_wip.tif"), overwrite = TRUE)
...
```{r}
hlef_soc_mask <- rast(paste0(soc_path,"hlef_soc", "_water_urban_mask", "no_wip.tif"))
hoh_soc_mask <- rast(paste0( soc_path,"hoh_soc", "_water_urban_mask", "no_wip.tif"))
mas_soc_mask <- rast(paste0( soc_path,"mas_soc", "_water_urban_mask", "no_wip.tif"))
col_soc_mask <- rast(paste0( soc_path,"col_soc", "_water_urban_mask", "no_wip.tif"))
...
C_map_simp <- function(C_map){
    ...
    return(data.frame("Name" = name, 
                      "Total_area" = area_tot, 
                      "AverageSOC_Mgha" = C_mean*100, 
                      #"MaxSOC_Mgha" = C_max,
                      "Total_SOC_Tg" = (TotalC_sum*100)/1e6,
                      stringsAsFactors = T))
}
...
readr::write_csv(mappedSOC_1m_depths_sum_df, here("data/dataframes/PNW_MapSOC_wetupl_1m_no_wip_no_wip.csv"))
```
...
SOC_df <- readr::read_csv(here("data/dataframes/PNW_MapSOC_wetupl_1m_nowip.csv"))

SOC_df |> 
  mutate(across(contains("SOC"), ~ . * 100)) |> 
  readr::write_csv(here("data/dataframes/PNW_MapSOC_wetupl_1m_Mgha_no_wip.csv"))
...
```{r }
#| eval: false
col_studyarea <- vect(here("data/Vectors/ColvilleHUC_2855.shp")) 
mas_studyarea <- vect(here("data/Vectors/Mashel_HUC12_2856.shp"))
hoh_studyarea <- vect(here("data/Vectors/HOH_POLYGON_711.shp"))
rastpath <- "data/Rasters/"

CONUS_NWCA_Mean <- rast("/Users/Anthony/OneDrive - UW/University of Washington/Data and Modeling/NWCA Data/CONUS_Stock_Nov_20/CONUS_Full_Stock_Mean.tif")
CONUS_NWCA_Max <- rast("/Users/Anthony/OneDrive - UW/University of Washington/Data and Modeling/NWCA Data/CONUS_Stock_Nov_20/CONUS_Full_Stock_Max.tif")
CONUS_NWCA_Min <- rast("/Users/Anthony/OneDrive - UW/University of Washington/Data and Modeling/NWCA Data/CONUS_Stock_Nov_20/CONUS_Full_Stock_Max.tif")
...
readr::write_csv(NWCA_df, here("data/dataframes/NWCA_SOC_C_map_simp.csv"))
...
```{r}
other_path <- here("data/Rasters/OtherSOC_sources/")
cr <- vect(here("data/Vectors/HLEF_Polygon_Filled.gpkg"))
hlef_study_area <- hlef_consolidate_lith |> tidyterra::filter(!is.na(LITH)) |> 
  crop(y = cr |> project("EPSG:6394"))
hoh_study_area <- vect(here("data/Vectors/HOH_POLYGON_711.gpkg")) |> 
  project("EPSG:2855")
mas_study_area <- vect(here("data/Vectors/Mashel_HUC12.shp")) |> terra::aggregate() |> project("EPSG:2856")
col_study_area <- vect(here("data/Vectors/ColvilleHUC_2855.shp")) |> terra::aggregate()
...
mcnicol_soc_frac_form <- mcnicol_soc_frac |> mutate(StudyArea = "HLEF") |> 
  dplyr::select(StudyArea, everything())
readr::write_csv(mcnicol_soc_frac_form, "data/dataframes/McNicol_2019_SOC_Frac_Estimates_HLEF.csv")
...
readr::write_csv(gsoc_frac_pnw, "data/dataframes/GSOC_Frac_Estimates_PNW.csv")
```
...
readr::write_csv(ensemble_PNW, "data/dataframes/ensemble_Frac_Estimates_PNW.csv")
```
...
soc_quantpred_func <- function(model, pred_stack, depth_ints, numcores) {
  stopifnot(is.vector(depth_ints))
  stopifnot(numcores < parallel::detectCores())
  # `model` must be the underlying ranger object (e.g. QRFM_stocks_nw$model$model)
  # trained with keep.inbag = TRUE; the 0.025/0.975 quantiles come from predict.ranger.
  name <- str_extract(deparse(substitute(pred_stack)), "hlef|hoh|mas|col")
  # pull $predictions out of the ranger.prediction list for terra (incl. cores > 1)
  ranger_pred <- function(model, ...) predict(model, ...)$predictions

  rast_low_list <- list()
  rast_high_list <- list()
  for(i in 1:length(depth_ints)) {
    rast_low_list[[i]] <- predict(pred_stack, model, fun = ranger_pred,
                type = "quantiles", quantiles = 0.025,
                const = data.frame(lower_depth = depth_ints[i]),
                na.rm = TRUE, cores = numcores, cpkgs = "ranger", wopt=list(steps=10))
    rast_high_list[[i]] <- predict(pred_stack, model, fun = ranger_pred,
                type = "quantiles", quantiles = 0.975,
                const = data.frame(lower_depth = depth_ints[i]),
                na.rm = TRUE, cores = numcores, cpkgs = "ranger", wopt=list(steps=10))
  }
  ...
  writeRaster(rast_95, 
              filename = paste0(soc_path, "PNW_", name, "_025_975_SOC_RFM_1m_nowip.tif"),
               overwrite = TRUE)
...
```{r}
#| eval: false
soc_quantpred_func(model = QRFM_stocks_nw$model$model,
              pred_stack = hlef_stack_select,
              depth_ints = seq(25, 100, 25),
              numcores = 6)
soc_quantpred_func(model = QRFM_stocks_nw$model$model,
...
```{r}
hlef_int_rast <- (rast("data/Rasters/SOC_Predictions/PNW_hlef_025_975_SOC_RFM_1m_nowip.tif"))
hoh_int_rast <- (rast("data/Rasters/SOC_Predictions/PNW_hoh_025_975_SOC_RFM_1m_nowip.tif"))
mas_int_rast <- (rast("data/Rasters/SOC_Predictions/PNW_mas_025_975_SOC_RFM_1m_nowip.tif"))
col_int_rast <- (rast("data/Rasters/SOC_Predictions/PNW_col_025_975_SOC_RFM_1m_nowip.tif"))
```
...
readr::write_csv(mappedSOC_025_1m_depths_sum_df, "data/dataframes/PNW_mappedSOC_025_1m_depths_sum_df_no_wip.csv")
readr::write_csv(mappedSOC_975_1m_depths_sum_df, "data/dataframes/PNW_mappedSOC_975_1m_depths_sum_df_no_wip.csv")
```
