...
features <- (names(QRFM_stocks_nw$model$model$variable.importance)[names(QRFM_stocks_nw$model$model$variable.importance) != "lower_depth"])
features
```
...
# writeRaster(hlef_stack_select, "data/Rasters/PredictorStacks/HLEF_PredictorStack_Class_RFM_select.tif")
...
#test <- soc_pred_func(QRFM_stocks_nw, hlef_stack_select, 25)
...
# soc_pred_func(model = QRFM_stocks_nw,
...
```{r}
hlef_soc <- rast(list.files(soc_path, pattern = "hlef", full.names = TRUE)) |> 
  sum() 

hoh_soc <- rast(list.files(soc_path, pattern = "hoh", full.names = TRUE)) |> 
  sum() 

mas_soc <- rast(list.files(soc_path, pattern = "mas", full.names = TRUE)) |> 
  sum() 

col_soc <- rast(list.files(soc_path, pattern = "col", full.names = TRUE)) |> 
  sum() 
plot(hlef_soc)
plot(hoh_soc)
plot(mas_soc)
...
```{r}
#| eval: false
soc_quantpred_func(model = QRFM_stocks_nw, 
              pred_stack = hlef_stack_select, 
              depth_ints = seq(25, 100, 25),
              numcores = 6)
soc_quantpred_func(model = QRFM_stocks_nw, 
