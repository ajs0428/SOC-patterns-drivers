...
to_remove <- RF_task$feature_names[str_detect(RF_task$feature_names, "_1000|_300|_50|nlcd|WIP")]
...
# instance$result
# 
# saveRDS(instance, file = "analysis/models/MLR3_RecursiveFeat_Stocks_noWIP.rds" |> here())
```



```{r}
#instance <- readRDS("analysis/models/MLR3_RecursiveFeat_Stocks_noWIP.rds" |> here())
...
saveRDS(tune_instance, file = "analysis/models/MLR3_RFM_Ranger_tune_instance_noWIP.rds" |> here())
```



```{r}
tune_instance <- readRDS(file = "analysis/models/MLR3_RFM_Ranger_tune_instance_noWIP.rds" |> here())
...
saveRDS(RF_lrn_tuned, "analysis/models/MLR3_RFM_Ranger_tuned_noWIP.rds" |> here())
...
```{r}
#| eval: false
RF_lrn_tuned <- readRDS("analysis/models/MLR3_RFM_Ranger_tuned_noWIP.rds" |> here())

RF_CV <- rsmp("repeated_cv", repeats = 5, folds = 5)

RF_rr <- mlr3::resample(RF_task, RF_lrn_tuned, RF_CV)
saveRDS(RF_rr, file = "analysis/models/MLR3_RFM_Ranger_resampling_noWIP.rds" |> here())
```


```{r}
RF_rr <- readRDS(file = here("analysis/models/MLR3_RFM_Ranger_resampling_noWIP.rds"))
...
write_csv(RF_rr_acc, file = "data/dataframes/MLR3_RFM_Ranger_tuned_CV_results_noWIP.csv" |> here())
```


```{r}
RF_rr_acc <- read.csv("data/dataframes/MLR3_RFM_Ranger_tuned_CV_results_noWIP.csv" |> here())
...
write_csv(tdf, here("data/dataframes/PNW_SOC_RF_noWIP_test_pred_response.csv"))
ggplot() +
  geom_point(aes(x = tdf$truth, y = tdf$predict)) +
...
saveRDS(ks, file = "analysis/models/MLR3_RFM_Ranger_KernelShap_noWIP.rds" |> here())
saveRDS(ks_wet, 
...
```{r}
ks <- readRDS(file = "analysis/models/MLR3_RFM_Ranger_KernelShap_noWIP.rds" |> here())
ks_wet <- readRDS(file = "analysis/models/MLR3_RFM_Ranger_KernelShap_Wet_noWIP.rds" |> here())
ks_upl <- readRDS(file = "analysis/models/MLR3_RFM_Ranger_KernelShap_Upl_noWIP.rds" |> here())
...
predictor <- iml::Predictor$new(RF_lrn_tuned,
                                data = RF_task_feats, 
                                y = RF_task_target)
saveRDS(predictor, "analysis/models/MLR3_Predictor_noWIP.rds" |> here())
...
saveRDS(ia_1_plot, file = "writing/Figures/MLR3_RFM_Interact_1_plot_noWIP.rds" |> here())
saveRDS(ia_2_plot, file = "writing/Figures/MLR3_RFM_Interact_2_plot_noWIP.rds" |> here())
saveRDS(ia_3_plot, file = "writing/Figures/MLR3_RFM_Interact_3_plot_noWIP.rds" |> here())
saveRDS(ia_4_plot, file = "writing/Figures/MLR3_RFM_Interact_4_plot_noWIP.rds" |> here())
saveRDS(ia_5_plot, file = "writing/Figures/MLR3_RFM_Interact_5_plot_noWIP.rds" |> here())
```
...
readr::write_csv(qrf_errdf, file = "data/dataframes/MLR3_RFM_ErrorDF_noWIP.csv" |> here())
```
