...
HLEF_ConfMat <- readRDS(here("analysis/models/HLEF_RF_Model_Final_ConfMat_stats.rds"))
HLEF_Model <- get(load(here("analysis/models/HLE_RF_Model_Final.RData")))

HRW_ConfMat_only <- HLEF_ConfMat$table
HRW_ConfMat_only[1,] <- c(190, 14)
HRW_ConfMat_only[2,] <- c(10, 85)
HRW_Model <- get(load(here("analysis/models/Hoh_2022_fullmodel_v08.RFmodel")))$model

MRW_ConfMat <- get(load(here("analysis/models/MAS_RF_Model_Final_ConfMat_stats.rda")))
MRW_Model <- get(load(here("analysis/models/MAS_RF_Model_Final.RData")))

CVW_ConfMat <- get(load(here("analysis/models/COL_RF_Model_Final_ConfMat_stats.rda")))
CVW_Model <- get(load(here("analysis/models/COL_RF_Model_Final.RData")))
...
```{r}
HRW_Kappa = caret::confusionMatrix(as.table(HRW_ConfMat_only))
...
All_WIP_Acc_DF <- data.frame(StudyArea = c("HLEF", "HRW", "MRW", "CVW"),
                             ...
                             Kappa = c(HLEF_ConfMat$overall[[2]], HRW_Kappa$overall[2][[1]][[1]], 
...
readr::write_csv(All_WIP_Acc_DF, here("data/dataframes/All_WIP_Acc_DF.csv"))
```
...
readr::write_csv(allsite_WIPimp, file = here("data/dataframes/PNW_WIP_VarImp.csv"))


```
```{r}
wipPredictors <- as_tibble(importance(HLEF_Model), rownames = "Predictor") |> 
    select(Predictor) |> 
    mutate(StudyArea = "HLEF") |> 
    bind_rows(as_tibble(importance(HRW_Model), rownames = "Predictor") |> 
                 select(Predictor) |> 
                 mutate(StudyArea = "HRW")) |> 
    bind_rows(as_tibble(importance(MRW_Model), rownames = "Predictor") |> 
                 select(Predictor) |> 
                 mutate(StudyArea = "MRW")) |> 
     bind_rows(as_tibble(importance(CVW_Model), rownames = "Predictor") |> 
                 select(Predictor) |> 
                 mutate(StudyArea = "CVW")) 

readr::write_csv(wipPredictors, here("data/dataframes/PNW_WIP_Predictors.csv"))    
```
