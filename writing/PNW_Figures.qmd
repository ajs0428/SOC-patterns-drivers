...
theme_param <- theme(
    ...
    legend.text = element_text(size = rel(0.90), hjust = 0.5),
...
coef_plots <- (coef_fig_lmm_stocks + ggtitle("Linear Mixed") + ylab(expression(SOC~Stock~g~cm^{2}))) +
  ...
  (coef_fig_lmm_lab + ylab(expression(Mineral~SOC~"%"))) +
  qrf_lab_shap + 
...
shap_lab_ph_map_plot <- sv_dependence(shapviz(ks_lab), 
                                    v = c(ks_lab_vars$column[4]), 
                  color_var = ks_lab_vars$column[1])
...
dep <- dep |> mutate(site = factor(site, levels = names(site_vals)))
stopifnot(!anyNA(dep$site))

shap_site_plot <- ggplot(dep, aes(feature_val, shap, color = site, fill = site)) +
  geom_point(shape = 21, size = 1.75, colour = "black", alpha = 0.7) +
  geom_line(stat = "smooth", linewidth = 1.1, alpha = 0.8, se = FALSE, method = "loess", formula = y ~ x, show.legend = F) +
...
shap_1_2_pdp_plot <- shap_pdp_plot_func(shap_1_2_plot, 
                                        ...
                                        colour_pal = "YlGnBu",
                                        xlab = "Depth (cm)") + 
                                          theme(legend.text = element_text(size = rel(0.90), hjust = 0.5))
...
shap_lab_ph_map_pdp_plot <- shap_pdp_plot_func(shap_lab_ph_map_plot, 
                                        name = "Depth (cm)", 
                                        colour_pal = "Blues",
                                        xlab = "MAP")+ 
                                          theme(legend.text = element_text(size = rel(0.90), hjust = 0.5))
...
shap_interact_pdp_wpH <- shap_site_plot   + shap_1_2_pdp_plot +
  shap_2_3_pdp_plot +  shap_lab_1_2_pdp_plot  +
  shap_lab_2_3_pdp_plot + shap_lab_ph_map_pdp_plot +
  plot_layout(nrow = 3, ncol = 2, axis_titles = "collect_y", guides = "collect")  + 
  plot_annotation(tag_levels = list(c("A.", "B.","C.", "D.", "E.", "F."))) & 
    theme(plot.tag = element_text(size = 8, face = "bold"),
          plot.tag.position = c(0, 1.02))
shap_interact_pdp_wpH

saveRDS(shap_interact_pdp_wpH, file = "/Users/Anthony/OneDrive - UW/University of Washington/Data and Modeling/SOIL CARBON/All_WA/writing/Figures/shap_interact_pdp.rds")
```
