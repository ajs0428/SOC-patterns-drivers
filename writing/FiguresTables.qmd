...
anova_ci_7 <- read.csv(paste0("SOIL CARBON/All_WA/data/dataframes/anova7d1_confint95.csv"))
anova_ci_w <- read.csv(paste0("SOIL CARBON/All_WA/data/dataframes/anova_wet_confint95.csv"))
anova_ci_u <- read.csv(paste0("SOIL CARBON/All_WA/data/dataframes/anova_upl_confint95.csv"))
ci_anova_stocks <- read.csv(paste0("SOIL CARBON/All_WA/data/dataframes/anova_confint95_stocks.csv"))
ci_anova_stocks_wet <- read.csv( paste0("SOIL CARBON/All_WA/data/dataframes/anova_wet_confint95_stocks.csv"))
ci_anova_stocks_upl  <- read.csv(paste0("SOIL CARBON/All_WA/data/dataframes/anova_upl_confint95_stocks.csv"))
...
rf_dat <- wa_dat_scale |> dplyr::select(SOC_stock_spline, WIP, MAP, site, LITH,
                                 MAT, CHM, HLI, LITH, EVI, lower_depth,
                                 dev_1000, dev_300, dev_50, grad_1000, grad_300,
                                 grad_50) 
rf_dat_wet <- wa_dat_scale_wet |> dplyr::select(SOC_stock_spline, WIP, MAP, site, LITH,
                                 MAT, CHM, HLI, LITH, EVI, lower_depth,
                                 dev_1000, dev_300, dev_50, grad_1000, grad_300,
                                 grad_50) 
rf_dat_upl <- wa_dat_scale_upl |> dplyr::select(SOC_stock_spline, WIP, MAP, site, LITH,
                                 MAT, CHM, HLI, LITH, EVI, lower_depth,
                                 dev_1000, dev_300, dev_50, grad_1000, grad_300,
                                 grad_50) 
...
rf.test <- read.csv("SOIL CARBON/All_WA/data/dataframes/RFM_testData_scalepreds.csv") |> select(-X) |>
  dplyr::mutate(LITH = as.factor(LITH))
rf.train <- read.csv("SOIL CARBON/All_WA/data/dataframes/RFM_Train_data.csv") |> select(-X) |>
  dplyr::mutate(LITH = as.factor(LITH))
rf.testwet <- read.csv("SOIL CARBON/All_WA/data/dataframes/RFM_testwetData_scalepreds.csv")|> select(-X) |>
  dplyr::mutate(LITH = as.factor(LITH))
rf.testupl <- read.csv("SOIL CARBON/All_WA/data/dataframes/RFM_testuplData_scalepreds.csv")|> select(-X) |>
 dplyr::mutate(LITH = as.factor(LITH))
...
hoh_dat_scale <- wa_dat_scale |> dplyr::filter(site == "HOH")
mas_dat_scale <- wa_dat_scale |> dplyr::filter(site == "MAS")
col_dat_scale <- wa_dat_scale |> dplyr::filter(site == "COL")
...
fit_vs_var_plot <- function(data, model, x_var, color_var, shape_var){
  ...
  colorlevels <- setNames(viridis::viridis(n = length(unique(data[, color_var]))),
...
```{r predplot_fun, echo=FALSE, warning=FALSE, message=FALSE, out.width="100%"}
library(RColorBrewer)
predict_plot <- function(model, data, back_transform) {
  lims <- c("limlow" = 0.001, "limhi" = 4.5, 
            "R2x" = 0.1, "R2y" = 3.75, 
              "fitx" = 0.7, "fity" = 3.25, 
              "redx" = 1.3, "redy" = 3.75, "redend" = 1.75, 
              "t1x" = 2.5, "t1y" = 3.75, 
  ...
  if(back_transform == "yes"){
    ...
    rmse_ <- caret::postResample(fitted,
                                 sample_data)[[1]]
    coef_v <- rmse_/mean(sample_data)
    ...
    annotate("label", fill = "white", label.size = 0,
             label = paste("R^{2} == ", 
                           signif(r.sq(sample_data,
                                      fitted), 2)), 
             x = lims["R2x"][1], y = lims["R2y"][1], size = 2.5, parse = T, hjust = 0) +
    annotate("label", fill = "white", label.size = 0,
             label = paste("RMSE == ", 
                           signif(rmse_, 2)), 
             x = lims["R2x"][1], y = lims["R2y"][1]*0.90, size = 2.5, parse = T, hjust = 0) +
    annotate("label", fill = "white", label.size = 0,
             label = paste("RMSE:Mean ==",
                           signif(coef_v, 2)),
             x = lims["R2x"][1], y = lims["R2y"][1]*0.80, size = 2.5, parse = T, hjust = 0) +
...
row_label_1 <- wrap_elements(panel = textGrob('Wetlands', rot=90,gp=gpar(fontsize=14)))
row_label_2 <- wrap_elements(panel = textGrob('Uplands', rot=90, gp=gpar(fontsize=14)))

all_predact_graphs <- plot_spacer() + plot_spacer() + lgend_predact + plot_spacer() +
  row_label_1 +
   (predact_graphs_list_form[[2]] + ggtitle(expression("LMM"["Depths"]))) +
  (predact_graphs_list_form[[5]] + ggtitle(expression("RFM"["Depths"]))) +
  (predact_graphs_list_form[[8]] + ggtitle(expression("GLM"["Pedons"]))) +
  row_label_2 +
  predact_graphs_list_form[[3]] +
  predact_graphs_list_form[[6]] +
  predact_graphs_list_form[[9]] +
  plot_layout(nrow = 3, ncol = 4, axis_titles = "collect",
              heights = c(0.3,1,1), widths = c(0.1,1,1,1)) &
  theme(plot.title = element_text(hjust = 0.5, size = 12, face = "bold"))
all_predact_graphs

full_predact_graphs <- plot_spacer() + lgend_predact + plot_spacer() +
  #plot_spacer() + plot_spacer() + plot_spacer() +
  predact_graphs_list_form[[1]] + ggtitle(expression("LMM"["Depths"])) +
  predact_graphs_list_form[[4]] + ggtitle(expression("RFM"["Depths"])) +
  predact_graphs_list_form[[7]] + ggtitle(expression("GLM"["Pedons"])) +
  plot_layout(nrow = 2, ncol = 3, axis_titles = "collect",
              heights = c(0.2,1)) &
  theme(plot.title = element_text(hjust = 0.5, size = 12, face = "bold"))
full_predact_graphs
```
...
save(full_predact_graphs, file = "/Users/Anthony/OneDrive - UW/University of Washington/Data and Modeling/SOIL CARBON/All_WA/writing/Figures/full_predact_graphs.RData")

ggplot2::ggsave(plot = full_predact_graphs, filename = paste0("/Users/Anthony/OneDrive - UW/University of Washington/Data and Modeling/SOIL CARBON/All_WA/writing/Figures/full_predact_graphs.png"),
                width = 7.5, height = 5, units = "in", dpi = 500, bg = "white")
```
...
coef_fig_func <- function(anova_ci, model) {
  coef_fig <- anova_ci |> 
    left_join(y = (summary(model)$coefficients |> 
                       data.frame() |> 
                       tibble::rownames_to_column("Predictor")), join_by(Predictor)) |> 
    dplyr::select(Predictor, X2.5., Estimate, X97.5.) |>
    ...
    mutate(Predictor = case_when(str_detect(Predictor, "lower_depth") ~ 
                                 ...
                                 str_detect(Predictor, "grad_300") ~ 
                                     str_replace(Predictor, "grad_300", "Gradient 300m"),
                                 str_detect(Predictor, "dev_1000") ~ 
                                     str_replace(Predictor, "dev_1000", "Deviation 1000m"),
                                 str_detect(Predictor, "dev_300") ~ 
                                     str_replace(Predictor, "dev_300", "Deviation 300m"),
                                                .default = (Predictor))) |> 
    dplyr::mutate(Predictor = case_when(str_detect(Predictor, "Lithology glacial till") ~ 
                                     str_replace(Predictor, "Lithology glacial till and drift",
                                                 "Lithology Glacial"),
                                     str_detect(Predictor, "Lithology") ~ 
                                     tools::toTitleCase(as.character(Predictor)),
                                     .default = (Predictor)),
                  Predictor = as.factor(Predictor)) |> 
    ggplot(aes(y = fct_reorder(Predictor, -desc(abs(Estimate))), x = Estimate)) +
    geom_point() +
    geom_errorbar(aes(xmin = X2.5., xmax = X97.5.)) + 
    geom_vline(xintercept = 0, lty = 3, colour = "red") +
    scale_x_continuous(name = "Coefficient Estimate", n.breaks = 5) +
    scale_y_discrete(name = "Predictor", labels = scales::label_wrap(15)) 
...
```{r}
coef_fig7 <- coef_fig_func(anova_ci_7, mod7)

coef_fig7_full <- coef_fig7 + 
  annotate("text", x = -1, y = 10.7, label = "*", hjust = 0.5, size = 8, colour = "red") +
  annotate("text", x = -1, y = 9.7, label = "*", hjust = 0.5, size = 8, colour = "red") +
  annotate("text", x = -1, y = 8.7, label = "*", hjust = 0.5, size = 8, colour = "red") +
  annotate("text", x = -1, y = 7.7, label = "*", hjust = 0.5, size = 8, colour = "red") +
  annotate("text", x = -1, y = 5.7, label = "*", hjust = 0.5, size = 8, colour = "red") 

coef_fig7w <- coef_fig_func(anova_ci_w, modw)

coef_fig7u <- coef_fig_func(anova_ci_u, modu)


coef_plots <- (coef_fig7_full + xlim(-1,1)) + (coef_fig7w + xlim(-1,1)) + (coef_fig7u+ xlim(-1,1)) + 
    plot_layout(nrow = 1, ncol = 3, axis_titles = "collect",
                widths = c(1,1,1)) +
...
save(coef_plots, file = "/Users/Anthony/OneDrive - UW/University of Washington/Data and Modeling/SOIL CARBON/All_WA/writing/Figures/LMM_coef_plots_graphs.RData")
...
coef_fig_pedon <- coef_fig_func(anova_ci = ci_anova_stocks, model = GLM)
coef_fig_pedon_w <- coef_fig_func(anova_ci = ci_anova_stocks_wet, model = GLMw)
coef_fig_pedon_u <- coef_fig_func(anova_ci = ci_anova_stocks_upl, model = GLMu)

coef_fig_pedon_full <- coef_fig_pedon + 
  annotate("text", x = -2, y = 8.8, label = "*", hjust = 0.5, size = 8, colour = "red") +
  annotate("text", x = -2, y = 7.8, label = "*", hjust = 0.5, size = 8, colour = "red") +
  annotate("text", x = -2, y = 6.8, label = "*", hjust = 0.5, size = 8, colour = "red") +
  annotate("text", x = -2, y = 4.8, label = "*", hjust = 0.5, size = 8, colour = "red") +
  annotate("text", x = -2, y = 3.8, label = "*", hjust = 0.5, size = 8, colour = "red") +
  annotate("text", x = -2, y = 1.8, label = "*", hjust = 0.5, size = 8, colour = "red") 

coef_pedon_plots <- (coef_fig_pedon+ xlim(-2, 2)) + 
  (coef_fig_pedon_w + xlim(-4, 2)) + 
  (coef_fig_pedon_u +xlim(-2, 2)) + 
    plot_layout(nrow = 1, ncol = 3, axis_titles = "collect",
                widths = c(1,1,1)) +
    plot_annotation(tag_levels = list(c("A", "B", "C"))) &
    theme(legend.position = 'right', 
...
save(coef_pedon_plots, file = "/Users/Anthony/OneDrive - UW/University of Washington/Data and Modeling/SOIL CARBON/All_WA/writing/Figures/Pedon_coef_plots_graphs.RData")
```
...
shap_dependence[[4]] <- shap_dependence[[4]] + labs(x = "Gradient 300m")

shap_dependence_plot <- shap_dependence +
  plot_layout(nrow = 2, ncol = 2, axis_titles = "collect") +
  plot_annotation(tag_levels = list(c( "A", "B", "C", "D"))) &
  theme(panel.background = element_blank(),
        axis.ticks = element_blank(),
...
shap_names <- unique(shap_plot$data$feature) |> str_replace("grad_", "Gradient ") |>
  str_replace("dev_", "Deviation ") |>
  str_replace("lower_depth", "Depth") |>
  str_replace("LITH", "Lithology") |>
  str_replace("300", "300m") |>
  str_replace("1000", "1000m") 

rf_incMSE_shap_plot <- shap_plot + 
  scale_y_discrete(labels= rev(shap_names)) +
  theme(panel.background = element_blank(),
           panel.grid.major = element_line(colour = "grey80", linewidth = 0.2),
        axis.ticks = element_blank(),
        text = element_text(size = 8))

RFM_shap_pdp_plot <- rf_incMSE_shap_plot + shap_dependence_plot +
    patchwork::plot_layout(widths = c(1,2)) +
  plot_annotation(tag_levels = list(c( "A", "B", "C", "D", "E"))) 

save(shap_dependence_plot, file = "/Users/Anthony/OneDrive - UW/University of Washington/Data and Modeling/SOIL CARBON/All_WA/writing/Figures/RFM_shap_dependence.RData")
...
save(RFM_shap_pdp_plot, file = "/Users/Anthony/OneDrive - UW/University of Washington/Data and Modeling/SOIL CARBON/All_WA/writing/Figures/RFM_shap_pdp_plot.RData")
...
```{r}
ia_depth_plot <- get(load("SOIL CARBON/All_WA/writing/Figures/RFM_Interact_Depth_plot.RData"))
ia_map_plot <- get(load("SOIL CARBON/All_WA/writing/Figures/RFM_Interact_MAP_plot.RData"))
ia_wip_plot <- get(load("SOIL CARBON/All_WA/writing/Figures/RFM_Interact_WIP_plot.RData"))
ia_grad_plot <- get(load("SOIL CARBON/All_WA/writing/Figures/RFM_Interact_grad300_plot.RData"))

graph_factor_str_replace <- function(features){
  features <- features |> str_replace("grad_", "Gradient ") |>
  str_replace("dev_", "Deviation ") |>
  str_replace("lower_depth", "Depth") |>
  str_replace("LITH", "Lithology") |>
  str_replace_all("300", "300m") |>
  str_replace("1000", "1000m") 
  return(features)
}
ia_depth_plot$data$.feature <- graph_factor_str_replace(ia_depth_plot$data$.feature)
ia_map_plot$data$.feature <- graph_factor_str_replace(ia_map_plot$data$.feature)
ia_wip_plot$data$.feature <- graph_factor_str_replace(ia_wip_plot$data$.feature)
ia_grad_plot$data$.feature <- graph_factor_str_replace(ia_grad_plot$data$.feature)



ia_plots <- ia_depth_plot + ia_map_plot + ia_wip_plot + ia_grad_plot +
  plot_layout(nrow = 2, ncol = 2, axis_titles = "collect") &
  theme(panel.background = element_blank(),
        axis.ticks = element_blank(),
...
rf_shap_interact_plot <- rf_incMSE_shap_plot + ia_plots +
    patchwork::plot_layout(widths = c(1,2)) +
  plot_annotation(tag_levels = list(c( "A", "B", "C", "D", "E")))

save(rf_shap_interact_plot, file = "SOIL CARBON/All_WA/writing/Figures/RFM_Shap_Interact_plot.RData")
```


### All variable importance (3 models) 

```{r}
all_model_var_imp_plot <- (coef_fig7_full + xlim(-1,1)) + ggtitle(expression("LMM"["Depths"])) +
  rf_incMSE_shap_plot + ggtitle(expression("RFM"["Depths"])) +
  (coef_fig_pedon_full+ xlim(-2, 2)) + ggtitle(expression("GLM"["Pedons"])) +
    plot_layout(nrow = 1, ncol = 3, axis_titles = "collect",
                widths = c(1,1,1)) &
    theme(plot.title = element_text(hjust = 0.5, size = 12, face = "bold"),
...
save(all_model_var_imp_plot, file = "SOIL CARBON/All_WA/writing/Figures/all_model_var_imp_plot.RData")
```
